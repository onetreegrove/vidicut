...
> 本文件由所有平台共享。具体检查标准因平台而异，见文末「平台阈值差异」表。

---

## 运行方式

本质检框架在写作完成后**静默执行**，对用户不可见。

- 不输出中间过程
- 不显示逐项检查日志
- 只有两种结果：**静默通过**（直接输出最终文章）或 **Cap-hit 报告**（见下）
...
L1 → L2 → L3 → L4
 ↑
任意层不通过 → 自动修复 → 从 L1 重新开始
```

每一层检查不通过，立即自动修复相关段落，然后从 L1 重新跑完整流程。不跳层，不跳过，必须全部通过才算达标。

---

## 迭代上限

最多进行 **3 轮完整迭代**（L1→L2→L3→L4 为一轮）。

3 轮后仍有未通过项，触发 Cap-hit 报告，输出当前最佳版本。
...
| 检查层 | 加载文件 |
|--------|----------|
| L1 | `writing-engine/forbidden-words.md` |
| L2 | + `writing-engine/style.md`（对应平台 style 文件） |
| L3 | + `writing-engine/*-prototypes.md`（对应平台原型文件） |
| L4 | 全量通读，无新增文件 |
...
这一层检查文章是否符合写作风格模式，类似代码的单元测试。每一项都给出"是/否"判断。
...
- 是否有一句话独立成段的"断裂"效果？（平台阈值见文末表格）
- 偏离主线的段落后面是否有"扣主线句"拉回来？
- 有没有用疑问句来制造节奏的刹车和转向？
- 是否避免了不必要的小标题？（除非是分条目的方法论文章，否则不应出现 markdown 标题或加粗小标题，靠口语化转场自然衔接）

**L2-3 口语化检查**
- 是否使用了推荐口语化词组？（全文口语化表达数量见平台阈值表）
- 有没有论述中的故意打破？（重复强调、中途打断、省略主语等）
- 有没有至少一处自嘲或承认不足？
- 标点是否用来表达情绪？（平台阈值见文末表格）
...
**修复方式**：这一层没有机械的修复方法。核心操作是，把"AI味重"的段落找出来，想象作者本人会怎么说这段话，然后用更口语、更私人、更不完美的方式重写。
...
| 检查项 | 公众号 | 小红书 | 口播 |
|--------|--------|--------|------|
| L2-2 一句话独立成段 | 最少 3 次 | 最少 2 次 | N/A |
| L2-3 口语化表达 | 8-10 个 | 4-6 个 | 8+ 个 |
| L2-3 情绪标点 | 必须 | 可选 | 可选 |
| L3-3 文化升维 | 必须 | 可选 | 可选 |
